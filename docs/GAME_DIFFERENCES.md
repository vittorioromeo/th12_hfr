...
| | TH08 | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 | New Classic |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Runtime | x86 | x86 | x86 | x86 | x86 | x86 | x86 | x86 | x64 (`hfr64.c`) |
| Graphics | D3D8 → d3d8to9 (`d3d8`) | D3D9 | D3D9 | D3D9 | D3D9 | D3D9 | D3D9 | D3D9 | D3D11 |
| Loader | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dinput8.dll` | `dxgi.dll` |
| `d3dx` | `d3dx8.dll` (static) | `d3dx9_31` | `d3dx9_37` | `d3dx9_40` | `d3dx9_43` | `d3dx9_43` | `d3dx9_43` | `d3dx9_43` | — |
| Update structure | Chain of callbacks, walked by `th08_walk` | update runner | update runner | update runner | update runner | update runner | update runner | the game's own runner, wrapped at its entry, node call and exit (`runner_wrap`) | own lists |
| Simulation rate | 60 Hz fixed | display | display | display | display | display | display | display | 60 Hz fixed |
| Extra frames come from | prediction + interpolation (`install_presentation`) | sub-stepping | sub-stepping | sub-stepping | sub-stepping | sub-stepping | sub-stepping | sub-stepping | interpolation |
| INI section | `[fixed60]` | `[hfr]` | `[hfr]` | `[hfr]` | `[hfr]` | `[hfr]` | `[hfr]` | `[hfr]` | `[fixed60]` |
| Frame entry | `frame_original`, `update_only` callbacks | `frame_fn` + `frame_calls` | same | same | same | same | same | `frame_fn` + `frame_calls`; the catch-up tick is the adapter's (`update_only`) | own clock hooks |

## 2. What runs at the display's rate (`classes`)

| System | TH08 | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 | New Classic |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Player | opt-in (`subtick`) | sub | sub | sub | sub | sub | sub | sub | opt-in (`subtick`) |
| Enemy bullets | opt-in (`substep`) | sub | sub | sub | sub | sub | sub | sub | opt-in (`substep`) |
| Lasers | opt-in (`substep`) | 60 Hz | sub | sub | sub | sub | sub | sub | opt-in (`substep`) |
| Items | opt-in (`substep`) | sub | sub | sub | sub | sub | sub | sub | 60 Hz |
| Stage / background | camera smoothed | 60 Hz | sub | sub | sub | 60 Hz | 60 Hz | 60 Hz | interpolated |
| Sprite animation | interpolated quads | sub (World, UI) | sub | sub | sub | sub (Early, Late passes) | sub (Early, Late passes) | sub (Early, Late passes) | interpolated |
| Enemies | predicted quads | 60 Hz, **not interpolated** (`place_enemy` NULL) | interpolated | interpolated | interpolated | interpolated | interpolated | interpolated | interpolated |
| Player options | predicted quads | 60 Hz | 60 Hz | 60 Hz | 60 Hz | interpolated (`place_options`) | interpolated (`place_options`) | carried with the player between frames (`th20_option_display`) | interpolated |
| Sub-tick input (`addr.poll_input`, `addr.game_input`) | `th08_live_input` | yes | yes | yes | yes | yes | yes | yes, through the adapter's own poll (`poll_raw`) | own |
...
| Field | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `runner_arg` | stack | EBX | EBX | EBX | ECX | ECX | ECX |
| `remove_node_abi` | node first | node first | node first | runner first | runner is `this` | runner is `this` | — (the game removes its own nodes) |
| `frame_ctx_ecx`, `cleanup_this_ecx`, `screenshot_stack_arg`, `anm_get_vm_ecx` | 0 | 0 | 0 | 0 | 1 (newer compiler) | 1 | `frame_ctx_ecx` 1; the others unused |
| `frame_flag_value` | 1 | 1 | 1 | 1 | 2 | 2 | 2 |
| `runner_return8_ends` | 0 | 1 | 1 | 1 | 1 | 1 | 1 |
| `critical_flag_mask` | 0 (always locks) | `0x8000` | `0x8000` | `0xff` | `0xff` | `0xff` | — (the game's runner takes its own lock) |
| `layout.runner_ending` | — | `0x48` | `0x48` | `0x54` | `0x54` | `0x54` | — |
| `layout.node_arg` / `runner_next` | `+0x20` / — | `+0x20` / — | `+0x20` / — | `+0x24` / `+0x50` | `+0x24` / `+0x50` | `+0x24` / `+0x50` | — |
| `layout.enemy_list` | — | `+0x68` | `+0x68` | `+0xb0` | `+0xd0` | `+0x180` | `+0x10c` |
| `layout.input_width` / `input_size` / `focus_mask` | 2 / `0x6a` / 4 | 4 / `0x130` / 8 | 4 / `0x130` / 8 | 4 / `0x130` / 8 | 4 / `0x248` / 8 | 4 / `0x248` / 8 | 4 / — (`poll_raw` saves what it needs) / 8 |
| `layout.autofocus_frames` ("hold shot to focus") | — | 8 | 8 | 8 | 10 | 10 | 10; the option is bit `0x100` (`layout.autofocus_option`) |
| `mask_minor_player_edges` | 1 | 1 | 0 | 0 | 0 | 0 | 1 |
| `native_size_cycle` | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| Speed writes (`speed_sites`) | 12, mostly `mov` | 15 `fstp` | 15 `fstp` | 16 `fstp` | 12, SSE (`SpeedSrc` XMM/none) | 12, SSE | 16 calls to `Float::set`, hooked by the adapter (`speed_sites_own`) |
| Speed model | timers point at a shared speed; per-site lengths | one global float | one global float | one global float | one global float | one global float; a timer holds an index into a rate table whose entry 0 is that float | a `Float` object with a setter; a timer holds an index into a rate table whose entry 0 is that object |
| `sprite_round_sites` (internal resolution) | — | — | — | 4 `frndint` sites | — | — | — |
| `layout.replay_mode` (the replay manager's mode word) | `+0x10` | `+0x10` | `+0x10` | `+0x10` | `+0x10` | `+0x0c` | `+0x10` |
| `addr.latency_cmp` | — | yes | yes | yes | yes | yes | — |
| `provisional` | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| `runner_wrap` (the game keeps its runner; three hooks instead of a replacement) | 0 | 0 | 0 | 0 | 0 | 0 | 1 |
| `speed_sites_own` | 0 | 0 | 0 | 0 | 0 | 0 | 1 |
...
| | TH08 | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 | New Classic |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| HFR extension | none (state is stock) | yes | yes | yes | yes | yes | yes | yes | none |
| Save sites hooked (`replay_saves`) | — | 2, via `th10_replay_save` | 4 | 4 | 4 | 4 | 2 | 2, via `th20_replay_save` (a method with four arguments) | — |
| Play sites hooked (`replay_load_calls`) | — | 2, via an ESI/stack adapter | 2 | 2 | 2, EBX/ECX adapter | 2 | 2 | 1, via `th20_replay_load` | — |
| Peek sites that must stay unhooked | — | `0x429765` | see notes | see notes | 3 | 2 | 2 | 1 (`0x508ad8`) | — |
| Magic | `T8RP` | `t10r` | `t11r` | `t12r` | `t13r` | `t13r` (reused) | `t15r` | `t20r` | — |
| Directory (`data_dir`) | game folder | game folder | game folder | game folder | `%APPDATA%\ShanghaiAlice\th13\` | `...\th14\` | `...\th15\` | `...\th20\` | — |
| Desync trace (`trace_state`, `trace_dump`) | own (`th08_trace`) | — | — | — | — | yes | yes | `trace_state` | — |
...
| Kind | Helper | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Enemy hit-test guard reads the player's integer timer | `g_ptf_prev`/`g_ptf_cur` stub | `0x42863e` | `0x434814` | `0x439ef2` | `0x446888` | `0x451450` | `0x4594ff` | `0x4c04bf`: the game asks "did the player's timer change" itself (its own slow-motion gate); answered per frame by `th20_player_ticked` |
| Fixed-point player movement loses its remainder | `movement_ftol` / `movement_cvttss` | yes | yes | yes | yes | yes (SSE variant) | yes (SSE variant) | yes (SSE variant) |
| Per-call counters inside sub-stepped objects (bullet wait, item countdown, laser ex-wait and graze, option and death counters, shot behaviours) | `gate_block`, or `E_timer_unchanged` in a stub: run only when the object's integer timer changed | yes | yes | yes | yes | yes | yes | yes |
| Per-frame rates outside `MotionState` (shot acceleration and turn, item gravity, homing) | `emit_factor` (`fmul g_factor`) | yes | yes | yes | yes | yes | yes, and the graze slow-down recovery (`0x44017b`) | yes: shot turn and acceleration, and a spawning bullet's per-call movement (`0x485cd1`) |
| Script-constant `Timer::add` scaled by `dt` (shot cycle, ANM `wait`) | add `value × logical` | 2 sites | 2 sites | 2 sites | 2 sites | next two rows | next two rows | 5 calls of `Timer::operator+=`/`-=`, which scale by the speed (`th20_timer_jump`) |
| Shot-cycle rewind through the rate-scaled `timer_rewind` | rate-pointer swap around the call | row above | row above | row above | row above | `0x45101a` | `0x44b88f`: the rate lookup inside `timer_rewind` (no pointer to swap) | `0x505c5b`, `0x505d23` (`th20_timer_wrap`: the wrap also stands in for that frame's tick) |
| Shot countdown timer misses the boundary tick at inexact `dt` | boundary-tick stub | — | — | — (inverse test) | `0x4436b4` | `0x44e08d`, `0x45131b` | `0x454ec4`, `0x459156` | — (the game fires on "timer changed") |
...
| | TH08 | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 | New Classic |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Attribution | draw-chain walker | runner dispatch | runner dispatch | runner dispatch | runner dispatch | runner dispatch, VM on the stack (`vm_stack_arg`) | as TH14; the VM names its ANM by slot (`vm_slot_off`, `anm_table_off`, `anm_slots`) | runner dispatch; the node is behind an iterator and the ANM name inside a `std::string` (`emit_node`, `anm_name`) | own map |
| `world_prio` | 8 | 11 | 11 | 12 | 12 | 19 | 19 | 20 | — |
...
| | TH08 | TH10 | TH11 | TH12 | TH13 | TH14 | TH15 | TH20 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
algorithm with its existing prediction diagnostics and gameplay hooks.
