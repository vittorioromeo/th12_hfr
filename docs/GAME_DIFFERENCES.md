...
| Executables supported | one | one | one | one | one | one | one | one | one | two builds, one profile each: 2026-09-07 (`th06nc.c`) and the Steam update of 2026-09-25, built 2026-09-14 (`th06nc_0914.c`), whose inlined draw runner needs `draw_end_call`/`draw_end`, `post_update_size`/`post_update_skip` and `draw_node_modrm` ([notes](games/TH06NC_DEVNOTES.md#24-the-steam-update-of-2026-09-25)) |
