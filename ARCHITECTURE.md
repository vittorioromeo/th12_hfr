...
| Older-engine adapter | `src/games/th08.c`, `src/backends/d3d8.c`, `d3d8_bridge.cpp`, `third_party/d3d8to9` | TH08: the Direct3D 8 bridge, a walker for the TH06-era Chain on the shared frame scheduler, a 60 Hz simulation with the player's movement, bullets and lasers sliced at the display's rate and replays that carry it, and presentation for the rest — predicted playfield, rigid-quad smoothing, camera smoothing, dimming in the quad hook ([TH08_DEVNOTES.md](docs/games/TH08_DEVNOTES.md)) |
