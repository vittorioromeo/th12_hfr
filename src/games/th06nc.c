...
static const struct FixedSignature th06nc_signatures[] = {
    ...
    /* Sub-stepped projectiles (TH06NC_DEVNOTES section 13). The state switch, the off-screen counter, the
    ...
    /* The item pool's update, called from the projectile manager's prologue (section 15). */
    ...
    /* The per-bullet sprite VM step at the tail of the loop, and the stepper it calls
       (section 13). The stepper is also the laser loop's, already gated there. */
